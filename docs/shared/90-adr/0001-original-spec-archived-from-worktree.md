---
title: 원본 SPEC.md 아카이브 (2026-04-21 워크트리)
adr_number: "0001"
status: Archived
date: 2026-04-21
---

# ADR-0001. 원본 SPEC 아카이브 (워크트리 2026-04-21)

## Status
Archived (2026-04-21 기획서, 2026-08-31 문서 체계 구축 시 워크트리에서 복원).

## Context

`.claude/worktrees/agent-a88d4fd7/` 와 `agent-a8e2f6f4/` 에 바이트 동일 `SPEC.md`가 있었다. git 추적 문서가 아니었고 코드보다 4개월 앞선 기획이다. 00-COMMON §6 / 03-green-forest §C에 따라 **정본이 아니라 재료**로 아카이브한다.

## Decision

1. `SPEC.md` 본문 전체를 이 ADR에 보관한다 (`spec-2.md` / `spec-3.md` / `ANALYSIS.md`는 복원하지 않음 — 03 지시: SPEC.md만).
2. 런타임 SSOT는 코드다. 이 파일은 역사 기록이다.
3. 워크트리 디렉토리는 아카이브 후 삭제한다.

## Consequences

**긍정**: 유일 기획 초안이 git에 남는다.
**부정**: 아래 본문은 2026-04-21 시점이다. 가챠·사진전·빙고·설문 등은 이후에 코드로 추가됐다.
**후속 작업**: 없음. 기능 추가는 코드를 본다.

## 코드와 대조 (2026-08-31, 살아있는 부분만)

컨트롤러·엔티티가 있는 영역: 게시/댓글/북마크/카테고리/JWT 로그인/대화(STOMP)/정원·퀘스트·랭킹/출석·가챠/설문/이벤트(빙고·사진전).

기획서에만 있고 이 작업에서 구현을 단정하지 않은 항목: 개인화 추천 알고리즘, 신고 관리 UI. 없으면 없는 것이고, 추측해서 문서를 만들지 않는다.
